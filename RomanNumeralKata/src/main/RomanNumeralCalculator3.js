function removeTwoLetterNumerals(numeral) {
  return numeral
    .replace(/IX/g, 'VIV')
    .replace(/IV/g, 'IIII')
    .replace(/XC/g, 'LXL')
...
function sortRomanNumeral(concatenatedNumerals) {
  return concatenatedNumerals
    .split('')
...
export default function add(numeral1, numeral2) {
  const expandedNumeral1 = removeTwoLetterNumerals(numeral1);
  const expandedNumeral2 = removeTwoLetterNumerals(numeral2);
